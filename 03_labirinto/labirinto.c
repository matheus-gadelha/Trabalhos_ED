...
#define pegar_vizinhos(l, c) {{l, c - 1}, {l - 1, c}, {l, c + 1}, {l + 1, c}}
...
bool valor_valido(int nl, int nc, char mat[nl][nc], int l, int c, char value){
...
bool furavel(int nl, int nc, char mat[nl][nc], int l, int c){
    if(!valor_valido(nl, nc, mat, l, c, '#')){ //Se não for parede, não faz nada
        return false;
    }

    int cont = 0;
    Posicao_vizinhos vizinhos[] = pegar_vizinhos(l, c);
    for(int i = 0; i < 4; i++)
        if(valor_valido(nl, nc, mat, vizinhos[i].l, vizinhos[i].c, '#'))
            cont++;
    if(cont < 3){
        return false;
    }
...
void furar_parede(int nl, int nc, char labirinto[nl][nc], int il, int ic, int fl, int fc){
    if(!furavel(nl, nc, labirinto, il, ic)){
        return;
    }
    
    labirinto[il][ic] = ' ';
    Posicao_vizinhos vizinhos[] = pegar_vizinhos(il, ic);
